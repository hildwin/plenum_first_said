...
`llm_classify.py` klassifiziert die nach den bestehenden Filtern verbliebenen Kandidatenwörter eines Protokolls in einem einzigen gebündelten Claude-API-Aufruf (Wortart + Lemma). Die Wortart filtert nichts aus dem Export heraus — Verben und Adjektive sind genauso interessante Funde wie Nomen —, sondern steuert nur die Lemma-Deduplizierung in `prune()` (pro Wortart getrennt, damit z.B. das Nomen "Bau" und das Verb "bauen" trotz ähnlichem Wortstamm als unabhängige Lemmata gelten). Schlägt der Aufruf fehl (Netzwerk, Rate-Limit, o.ä.), exportiert `prune()` die Kandidaten unverändert ohne Lemma-Abgleich weiter — der Tages-Export geht dadurch nie verloren.

Im Ordner `utilities` finden sich Hilfsskripte: `build_database_local.py` verarbeitet lokal in `parser/archive/` abgelegte Protokoll-XMLs in den Korpus (für den Erstaufbau, siehe unten). `download_new_format_xml.py` lädt Protokolle im neuen, reich strukturierten XML-Format automatisiert über die DIP-API herunter. `load_namen.py` befüllt den Namensfilter (siehe "Was bedeutet neues Wort?") aus den MdB-Stammdaten, wahlweise aus einer lokalen Datei oder per `--url` direkt von bundestag.de. `wort_herkunft.py` schlägt nach, in welcher Sitzung/Wahlperiode ein Wort im Korpus zuerst auftauchte; mit dem Flag `--dwds` wird zusätzlich `dwds_api.py` befragt, das über das DWDS-Korpus "Bundestagsprotokolle" (`dstar/bundestagprt`) unabhängig vom eigenen Korpus den frühesten dort erfassten Beleg desselben Wortes ermittelt - ein externer Vergleichswert, kein Ersatz für den eigenen Fund. `bereinige_korpus_zeichen.py` räumt rückwirkend die Zeichen-Artefakte auf (Geviertstrich, Halbgeviertstrich, Gradzeichen, Ellipse, öffnendes einfaches Anführungszeichen, weicher Trennstrich, geschützter Bindestrich), die im laufenden Betrieb nach und nach in `text_parse.py` behoben wurden, aber nur für künftig verarbeitete Dateien galten - per Default ein reiner Dry-Run, erst `--apply` schreibt tatsächlich. Nicht parallel zu einem laufenden Korpus-Aufbau/Live-Betrieb ausführen (Race-Condition-Risiko). `setze_meta_id.py` bestimmt einmalig die korrekte Start-ID für `plenar.py` (`meta:id`, die DIP-API-interne fortlaufende Dokument-ID - ein anderer ID-Raum als die WP+Sitzungsnummer-ID des Korpus): sucht die DIP-ID des zuletzt lokal per `build_database_local.py` verarbeiteten Protokolls und setzt `meta:id` auf diese ID + 1, damit der Live-Betrieb ab dem nächsten wirklich neuen Protokoll weitersucht statt bereits Bekanntes erneut zu verarbeiten - ebenfalls per Default ein reiner Dry-Run, erst `--apply` schreibt tatsächlich. `nacherfassung_wp21.py` holt rückwirkend nach, was `build_database_local.py` beim Erstaufbau verworfen hat: die pro WP21-Sitzung tatsächlich neu gefundenen Wörter (mit Satzkontext/Sprecherzuordnung) wurden dort nur gezählt, nie durch die LLM-Wortart/Lemma-Klassifikation geschickt oder exportiert. Prüft dafür rein lesend, ob die im Korpus hinterlegte "zuerst gesehen"-ID exakt der aktuell verarbeiteten Sitzung entspricht (statt `check_newness()`, das für den bereits vollständigen WP21-Bestand immer "schon bekannt" melden würde). In der Testphase Export in eine separate CSV statt `neue_woerter.csv`/`.db`, `merke_lemma()` wird nicht aufgerufen (kein Schreibzugriff auf die produktiven `lemma:*`-Keys).
...
- Wörter, deren Lemma (laut LLM-Klassifikation in `llm_classify.py`, getrennt pro Wortart) bereits bekannt ist (verhindert künftige Dopplungen wie z.B. Genitiv-/Plural-Varianten desselben Wortstamms) — dieser Abgleich läuft nur vorausschauend ab Einführung des Features, nicht rückwirkend gegen den bereits bestehenden historischen Korpus. Verben und Adjektive werden dabei nicht ausgefiltert, nur wie alle anderen Wörter auf Lemma-Dopplungen geprüft.
